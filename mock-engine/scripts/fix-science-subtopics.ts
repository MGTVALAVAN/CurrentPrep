...
const VALID_SUBTOPICS = ['sci-physics', 'sci-biology', 'sci-it', 'sci-defense'];

function main() {
  const files = fs.readdirSync(generatedDir)
    .filter(f => f.startsWith('science_') && f.endsWith('.json'));

  console.log(`\n🔧 Fixing Science sub-topic tags`);
  console.log(`   Found ${files.length} science question files\n`);

  let totalFixed = 0, totalQuestions = 0;

  for (const file of files) {
    const match = file.match(/^science_(sci-[a-z]+)_/);

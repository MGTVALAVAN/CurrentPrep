...
const VALID_SUBTOPICS = ['ca-national', 'ca-international', 'ca-awards', 'ca-reports'];

function main() {
  const files = fs.readdirSync(generatedDir)
    .filter(f => f.startsWith('current_affairs_') && f.endsWith('.json'));

  console.log(`\n🔧 Fixing Current Affairs sub-topic tags`);
  console.log(`   Found ${files.length} current affairs question files\n`);

  let totalFixed = 0, totalQuestions = 0;

  for (const file of files) {
    const match = file.match(/^current_affairs_(ca-[a-z]+)_/);

...
const VALID_SUBTOPICS = ['soc-structure', 'soc-women', 'soc-population', 'soc-issues'];

function main() {
  const files = fs.readdirSync(generatedDir)
    .filter(f => f.startsWith('society_') && f.endsWith('.json'));

  console.log(`\n🔧 Fixing Society sub-topic tags`);
  console.log(`   Found ${files.length} society question files\n`);

  let totalFixed = 0, totalQuestions = 0;

  for (const file of files) {
    const match = file.match(/^society_(soc-[a-z]+)_/);
